...
func Remind(cmd Command, tx *sql.Tx, db *sql.DB) string {
    _, err := tx.Exec(
...
func Notify(cmd Command, tx *sql.Tx, db *sql.DB) string {
    _, err := tx.Exec(
...
func GetTime(cmd Command, tx *sql.Tx, db *sql.DB) string {
    t := time.Now()
...
func AllReminders(cmd Command, tx *sql.Tx, db *sql.DB) string {
...
func AllNotifications(cmd Command, tx *sql.Tx, db *sql.DB) string {
...
func RmReminders(cmd Command, tx *sql.Tx, db *sql.DB) string {
    tx.Exec(fmt.Sprintf("delete from reminders where date<=%d", cmd.Created))
    return ""
}

func RmNotifications(cmd Command, tx *sql.Tx, db *sql.DB) string {
